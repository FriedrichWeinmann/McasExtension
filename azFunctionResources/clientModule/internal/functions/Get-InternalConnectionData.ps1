...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'McasExtension.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'McasExtension.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'McasExtension.Client.ProtectedToken'
